...
#1: parent lenght(cm)+median_distance+ rank_node+length(node)####
...
#3: parent lenght(cm)+median_distance####
...
better_perm=length(which(df$b==1))#times better perm!!!
#graph
png("3P_1.png",width=1200, height=900, res=150)
...
better_perm=length(which(df$b==1))#times better perm!!!
#graph
png("3P_2.png",width=1200, height=900, res=150)
plot(allEffects(test))
dev.off()

#i want to keep rank and distance because it makes more sense biologically
#5: rank_node####
data = PROL_bud_scale[PROL_bud_scale$rank_node<=16,]
#test
test=multinom(fate~rank_node,data = data)
summary(test)
exp(coef(test))
#computing p value
z <- summary(test)$coefficients/summary(test)$standard.errors
...
p 
#real value
tab=as.matrix(prop.table(table(data$rank_node,data$fate),1))
tab
#predicted
...
#graph
png("3P_rank.png",width=1200, height=900, res=150)# save plot
par(mar=c(5,5,5,5))
...
#do not like
#6: not happy with the graphs sq.root of rank####
...
#graph
png("3P_polinomial.png",width=1200, height=900, res=150)# save plot
par(mar=c(5,5,5,5))
cols<-brewer.pal(n=3,name="Set2")
r=barplot(t(tab),
          col = cols, names.arg = row.names(tab),
