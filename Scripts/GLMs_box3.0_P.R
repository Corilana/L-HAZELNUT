...
colnames(PROL_met_scale)[c(2,7,8,11)]=c("length_cm",
                                             "length_nodes",
                                             "rank_node",
                                             "distance")
PROL_met_scale$tot_buds=PROL_met_scale$tot_buds-PROL_met_scale$b#REMOVING THE COUNTING OF CATKINS BECAUSE ALL PROLLEPTIC HAS CATKINS

#1: m+v buds ~ parent lenght(cm)+distance+ rank_node?####
glm_box1=glm(tot_buds~length_cm+distance+rank_node,family = "poisson",data = PROL_met_scale)
summary(glm_box1)

#we have to choose if weeping distance or rank node or length
#rank
nline=length(unique(sort(PROL_met_scale$rank_node)))
mv=grep("tot", colnames(PROL_met_scale))
dt=met[0,0]
...
glm_box1=glm(tot_buds~length_cm+normal_distance+rank_node+length_nodes,family = "poisson",data = PROL_met_scale)
summary(glm_box1)#yes
#2: m+v buds ~ parent lenght(cm)+rank_node?####
glm_box1=glm(tot_buds~length_cm+rank_node,family = "poisson",data = PROL_met_scale)
summary(glm_box1)#yes
...
summary(glm_box1)#yes
#3:b~length+rank?####
glm_box1=glm(b~rank_node+length_nodes,family = "binomial",data = PROL_met_scale)
summary(glm_box1)#yes
#4:b~rank?####
...
PROL_met_scale[PROL_met_scale$rank_node<5,8]=0
PROL_met_scale[PROL_met_scale$rank_node>=5,8]=1
glm_box1=glm(tot_buds~length_cm+distance+rank_node,family = "poisson",data = PROL_met_scale)
summary(glm_box1)#yes
...
t.test(tot_buds~rank_node, data=PROL_met_scale)#yes
