...
library(RColorBrewer);library(plotrix);head(met.sylleptic)

source("Own_rooted_young/Scripts/GLMs/3_SYL_nb_mv_analysis.R")
...
for (i in 1:nline) {
  ...
  av = round(mean(met.sylleptic[met.sylleptic$parent_length_cm == L, "tot_buds_syl_m_v"]), digits = 2)
  n = length(met.sylleptic[met.sylleptic$parent_length_cm == L, "tot_buds_syl_m_v"])
  se = round(sd(met.sylleptic[met.sylleptic$parent_length_cm == L,
                              "tot_buds_syl_m_v"])/sqrt(length(!is.na(met.sylleptic[met.sylleptic$parent_length_cm == L, "tot_buds_syl_m_v"]))),
             digits =
               2)
  if (n == 1) {
    se = 0
...
#graph
# png("Own_rooted_young/Outputs/Plots/3_SYL_nb_buds.png",width=1200, height=900, res=150)# save plot
