...
source("Own_rooted_young/Scripts/GLMs/6_PRO_length_new_shoots_analyses.R")
...
for (i in levels(MV.bud.PRO$fate)) {
  ...
  for (j in levels(df$class_distance)) {
    ...
    for (z in unique(dt$length)) {
      length = z
      dx = dt[dt$length == z, ]
      av = mean(dx$length2yo)
      se = sd(dx$length2yo) / sqrt(length(dx$length2yo))
...
#graph
#png("Own_rooted_young/Outputs/Plots/6_PRO_length_new_shoots.png",width = 1200,height = 900,res = 150)# save plot
par(oma = c(4, 4, 2.5, 4))
