...
for (i in 1:4) {
  ...
  bud.type[i, "nb_v"] = sum(met.sylleptic[met.sylleptic$class == syl_class, "v_syl"])
  bud.type[i, "nb_m"] = sum(met.sylleptic[met.sylleptic$class == syl_class, "m_syl"])
  bud.type[i, "nb_b"] = sum(met.sylleptic[met.sylleptic$class == syl_class, "b"])
  bud.type[i, "nb_c"] = sum(met.sylleptic[met.sylleptic$class == syl_class, "c_syl"])
}
...
# pdf("Own_rooted_young/Outputs/Tables/4_SYL_mv~class.pdf",height = 3,width = 5 )
grid.table(bud.mv)
# dev.off()
...
# syl count
# png("Own_rooted_young/Outputs/Plots/4_SYL_freq_MV.png",width=1200, height=900, res=150)# save plot
