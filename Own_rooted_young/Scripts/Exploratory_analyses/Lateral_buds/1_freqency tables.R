...
for (i in levels(bud$class)) {
  ...
  for (q in levels(bud$class)) {
    class.newshoot[i, paste0("new_shoot_", q)] = sum(bud[bud$class == i &bud$length_newshoots == q, "nb_new_shoots"], na.rm = T)
  }
...
# pdf("Own_rooted_young/Outputs/Tables/freq_class_length.pdf", height = 4, width = 13)
...
#write pdf with the table
# pdf("Own_rooted_young/Outputs/Tables/freq_shoot_type.pdf",height = 4,width = 13)
...
#write pdf with the table
# pdf("Own_rooted_young//Outputs/Tables/SYL_obs~rank.pdf",height = 8,width = 10 )
grid.table(TAB_SYL)
# dev.off()
