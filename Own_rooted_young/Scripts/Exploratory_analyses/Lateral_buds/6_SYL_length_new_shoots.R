...
library(RColorBrewer);library(pals);library(gridExtra);library(tidyr)

source("Own_rooted_young/Scripts/Modify_dataset/import_dataset_OR.R")
# Length from M and V -----------------------------------------------------
#visualize plot
str(MV.bud.SYL)
plot(length2yo~parent_length_cm,MV.bud.SYL,pch=19)
plot(length2yo~parent_rank_node,MV.bud.SYL,pch=19)
plot(length2yo~abs_norm_median_distance,MV.bud.SYL,pch=19)
plot(length2yo~abs_median_distance,MV.bud.SYL,pch=19)
plot(length2yo~fate,MV.bud.SYL,pch=19)
plot(length2yo~m,MV.bud.SYL,pch=19)
plot(length2yo~v,MV.bud.SYL,pch=19)
plot(length2yo~siblings_mv, MV.bud.SYL, pch=19)
plot(length2yo~norm_median_distance,MV.bud.SYL,pch=19)
#seems not to be correlated
#frequency table of combination neweral child in sylleptic shoots per each rank node
lne = grep("length_newshoots", colnames(MV.bud.SYL))#adress the column with the class of the child
...
print(new_class)
#graph
# png("Own_rooted_young/Outputs/Plots/6_SYL_fromMV_newshootclass.png",width = 1200,height = 900,res = 150)# save plot
par(mar = c(5, 5, 4, 10))
...
#graph
# png("Own_rooted_young/Outputs/Plots/6_SYL_fromMV_freq_newshootclass.png",width = 1200,height = 900,res = 150)# save plot
par(mar = c(5, 5, 4, 10))
cols <- rev(watlington(n = length(colnames(t(
  new_class.freq
)))))
...
par_child = as.data.frame.matrix(table(bud.sylleptic$class, bud.sylleptic$length_newshoots))
...
# pdf("Own_rooted_young/Outputs/Tables/SYL_newlengh~prollengh.pdf", height = 5,width = 12)
