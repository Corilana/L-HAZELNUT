...
source("Own_rooted_young/Scripts/GLMs/5_PRO_new_shoots_proba_analyses.R")

library(plotfunctions);library(RColorBrewer);library(plotrix)

#create categories for median distance norm
summary(MV.bud.PRO$nb_new_shoots)
...
#graph
# png("Own_rooted_young/Outputs/Plots/5_PRO_proba_new_shoots_glm.png",width = 1200,height = 900,res = 150)# save plot
par(oma = c(4, 4, 2.5, 4))
...
# legend("topright", pch = c(19,NA),lty=c(NA, 1),cex=0.8,legend = c("observed", "predicted"))
